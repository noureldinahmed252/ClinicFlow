  ...
  tbody.innerHTML = filtered
    .map((p, idx) => {
      const lastVisit =
        ...
        "";
      const patientIdArg = escapeJsString(p.id);
      const safeInitials = escapeHtml(getInitials(p.name || ""));
      const safeName = escapeHtml(valueOrDash(p.name));
      const safePatientId = escapeHtml((p.id || "").slice(-6));
      const safeAge = escapeHtml(valueOrDash(p.age));
      const safeGender = escapeHtml(valueOrDash(p.gender));
      const safePhone = escapeHtml(valueOrDash(p.phone));
      const safeBloodType = escapeHtml(p.bloodType);
      const safeLastVisit = lastVisit ? escapeHtml(formatDate(lastVisit)) : "";
      return `<tr style="animation-delay:${idx * 0.04}s;">
      <td><span class="row-num">${idx + 1}</span></td>
      <td>
        <div class="patient-cell">
          <div class="patient-initials">${safeInitials}</div>
          <div>
            <div class="patient-name">${safeName}</div>
            <div class="patient-id">#${safePatientId}</div>
          </div>
        </div>
      </td>
      <td>${safeAge}y / <span style="text-transform:capitalize;">${safeGender}</span></td>
      <td style="font-family:'JetBrains Mono',monospace;font-size:13px;">${safePhone}</td>
      <td>${p.bloodType ? `<span class="badge badge-confirmed">🩸 ${safeBloodType}</span>` : '<span style="color:var(--text-muted);">-</span>'}</td>
      <td>${lastVisit ? `<span style="font-size:12.5px;">${safeLastVisit}</span>` : '<span style="color:var(--text-muted);">-</span>'}</td>
      <td>
        <div style="display:flex;gap:5px;">
          <button class="btn btn-secondary" onclick="viewPatient('${patientIdArg}')" style="padding:6px 10px;font-size:12px;" title="View full record">
          ...
          <button class="btn btn-ghost" onclick="openEditPatient('${patientIdArg}')" style="padding:6px 10px;font-size:12px;" title="Edit">
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
          </button>
          <button class="btn btn-danger" onclick="deletePatient('${patientIdArg}')" style="padding:6px 10px;font-size:12px;" title="Delete">
...
function openEditPatient(id) {
  ...
  const rawPatient = SafeDB.get("patients").find((x) => x.id === id);
  // 🔐 Only allow editing if patient belongs to current doctor
  if (!rawPatient || rawPatient.doctorId !== currentDoctorId) {
    toast("Patient not found or access denied.", "warning");
    return;
  }
...
function savePatient(e) {
  ...
  const patients = SafeDB.get("patients").map(normalizePatient);
  const existing = id
    ? patients.find((x) => x.id === id && x.doctorId === currentDoctorId)
    : null;
  const p = {
...
function deletePatient(id) {
  ...
  const patientToDelete = SafeDB.get("patients").find((x) => x.id === id);
  if (!patientToDelete || patientToDelete.doctorId !== currentDoctorId) {
    toast("Cannot delete: patient not found or access denied.", "warning");
    return;
  }
  const patients = SafeDB.get("patients").filter((x) => x.id !== id);
  const appts = SafeDB.get("appointments")
    .map(normalizeAppointment)
    .filter((a) => a.patientId !== id);
  const consultations = SafeDB.get("consultations").filter(
    (c) => c.patientId !== id,
  );
  SafeDB.set("patients", patients);
...
function viewPatient(id) {
  ...
  console.log("VIEW PATIENT DEBUG", {
    rawPatient,
    normalizedPatient: p,
    currentDoctorId,
  });
  // 🔐 Only show patient if they belong to current doctor
  if (!p || p.doctorId !== currentDoctorId) {
    toast("Patient not found or access denied.", "warning");
    return;
  }
  const appts = SafeDB.get("appointments")
  ...
  const consultations = SafeDB.get("consultations")
    .filter((c) => c.patientId === id && c.doctorId === currentDoctorId)
    .map(normalizeVisit);
  if (!p) return;
  ...
  const safeAge = escapeHtml(valueOrDash(p.age));
  const safeGender = escapeHtml(valueOrDash(p.gender));
  const safeCity = escapeHtml(valueOrDash(p.city));
  const safePhone = escapeHtml(valueOrDash(p.phone));
  const safeBloodType = escapeHtml(valueOrDash(p.bloodType));
  const safeConditions = conditions.map((c) => escapeHtml(c));
  const safeConditionsText = safeConditions.join(", ") || "-";
  const safeBmi = bmi ? escapeHtml(bmi.bmi) : "";
  const safeBmiLabel = bmi ? escapeHtml(bmi.label) : "";

  document.getElementById("patientDetailsContent").innerHTML = `
    <div class="patient-detail-hero">
      <div style="display:flex;align-items:center;gap:24px;flex-wrap:wrap;">
        <div style="width:80px;height:80px;background:linear-gradient(135deg,#0ea5e9,#0284c7);border-radius:20px;display:flex;align-items:center;justify-content:center;font-size:36px;font-weight:700;color:white;">${safeInitials}</div>
        <div>
          <h2 style="font-family:'Fraunces',serif;font-size:28px;font-weight:700;">${safeName}</h2>
          <p style="color:#94a3b8;margin-top:4px;">${safeAge} years · ${safeGender} · ${escapeHtml(marriedLabel(p.married))} · ${safeCity}</p>
          <div style="display:flex;gap:10px;margin-top:12px;flex-wrap:wrap;">
            ${safeConditions.map((c) => `<span class="badge badge-pending">${c}</span>`).join("")}
            ${p.bloodType ? `<span class="badge badge-confirmed">🩸 ${safeBloodType}</span>` : ""}
            ${bmi ? `<span class="bmi-display ${bmi.cls}">${safeBmi} BMI — ${safeBmiLabel}</span>` : ""}
          </div>
        </div>
        <div style="margin-left:auto;display:flex;gap:10px;">
          <button class="btn btn-primary" onclick="openEditPatient('${patientIdArg}');showApp('patients')">✏️ Edit</button>
          <button class="btn btn-success" onclick="openNewApptForPatient('${patientIdArg}')">➕ Appointment</button>
        </div>
      </div>
    </div>
    <div class="info-grid">
      <div class="info-card">
        ...
        <div class="info-item"><div class="info-icon">📱</div><div><div class="info-label">Phone</div><div class="info-value">${safePhone}</div></div></div>
        <div class="info-item"><div class="info-icon">📍</div><div><div class="info-label">City</div><div class="info-value">${safeCity}</div></div></div>
        <div class="info-item"><div class="info-icon">💍</div><div><div class="info-label">Status</div><div class="info-value">${escapeHtml(marriedLabel(p.married))}</div></div></div>
      </div>
      <div class="info-card">
        ...
        <div class="info-item"><div class="info-icon">📊</div><div><div class="info-label">BMI</div><div class="info-value">${bmi ? `${safeBmi} (${safeBmiLabel})` : "-"}</div></div></div>
      </div>
      <div class="info-card">
        <div class="info-card-title">🩺 Medical Info</div>
        <div class="info-item"><div class="info-icon">🩸</div><div><div class="info-label">Blood Type</div><div class="info-value">${safeBloodType}</div></div></div>
        <div class="info-item"><div class="info-icon">📋</div><div><div class="info-label">Conditions</div><div class="info-value">${safeConditionsText}</div></div></div>
      </div>
    </div>
      ...
      ${
        ...
        <div class="appointment-item">
          ...
          <div class="appt-info">
            ...
            <div class="appt-meta">${escapeHtml(a.notes || "No notes")}</div>
          </div>
          ${badgeHtml(a.status)}
        </div>`,
              )
    ...
    <div class="card">
      ...
      ${
        ...
        <div class="visit-item">
          ...
          <div class="visit-diagnosis">${escapeHtml((c.diagnoses || []).map((d) => d.name || d).join(", ") || "-")}</div>
          <div style="display:flex;gap:8px;margin-top:6px;flex-wrap:wrap;">
            ${(c.medications || []).map((m) => `<span class="badge badge-confirmed">💊 ${escapeHtml(m.name)}</span>`).join("")}
          </div>
          <div class="visit-condition" style="color:var(--text-muted);">${escapeHtml(c.notes || "")}</div>
        </div>`,
              )
              .join("")
