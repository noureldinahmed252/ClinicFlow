...
class AuthValidator {
  static isValidToken(token) {
    ...
    try {
        ...
        // لازم يكون فيه تاريخ انتهاء
        if (!payload.exp) {
            return false;
        }

        // التأكد إن التوكن لسه صالح
  ...
  static getValidToken() {
    ...
    // If no token exists in storage, return null silently (normal state)
    if (token === null) {
      return null;
    }
    // Token exists but validation failed - remove corrupted token and warn
    if (!this.isValidToken(token)) {
      console.warn("Invalid stored token removed");
